...
class RefreshToken(db.Model):
    ...
    def __init__(self, token: str) -> None:
        ...
        Parameters:
            refresh_token (str): A refresh token containing a specific
                                 user's data.
        """
        self.token = token
    ...
    def to_json(self) -> dict:
        ...
        logger_manager.info("Token's data successfully fetched")
        return json_user
...
def get_refresh_token_by_id(id: uuid.UUID) -> RefreshToken:
    ...
    Parameters:
        id (uuid): The user's ID stored in the token.

    Returns:
        refresh_token (RefreshToken): The refresh token of the current
                                      session's user.
    """
    try:
        refresh_token_list = RefreshToken.query.all()
        for refresh_token in refresh_token_list:
            token = decode_token(refresh_token.token)
...
def get_refresh_token(token: str) -> RefreshToken:
    ...
    Parameters:
        token (str): The refresh token to check.
