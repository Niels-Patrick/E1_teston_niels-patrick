...
def test_add_game(client):
    ...
    test_event: Event = Event.query.filter_by(name='test').first()
    ...
    game = {
        ...
        "idPlayerBlack": get_player_by_username("tuser").id_player
    }
...
def test_get_game(client):
    game: Game = Game.query.filter_by(
        id_player_white=get_player_by_username("tuser").id_player
        ).first()
